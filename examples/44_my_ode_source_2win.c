...
int main(int argc, char **argv)
{
  ...
  qsApp->op_fadePeriod = 6.0F; // seconds
  ...
  ms->rk4 = qsRungeKutta4_create(
    ...
    0 /* size  op_bgR, op_bgG, op_bgBof object to allocate, 0 => default */);
 ...
 sweep = qsSweep_create(4.0F /*sweep period, seconds*/,
  ...
  /* change the fading for a another win.  Here more stuff you can set
   * for a win. */
  // fade, fadePeriod, and fadeDelay are properties of the win.
  qsApp->op_fadePeriod = 7.0F; // seconds
  qsApp->op_fadeDelay = 5.0F; // seconds
  qsApp->op_bgR = 0.02F;
  qsApp->op_bgG = 0.2F;
  qsApp->op_bgB = 0.0F;
  qsApp->op_subGrid = true;
  qsApp->op_subGridR = 0.1F;
  qsApp->op_subGridG = 0.5F;
  qsApp->op_subGridB = 0.1F;
  qsApp->op_axis = true;
  qsApp->op_grid = true;
  qsApp->op_gridR = 0.6F;
  qsApp->op_gridG = 0.1F;
  ...
  qsApp->op_width = 300;
  qsApp->op_height = 300;
  /* create a new win with the new fade parameters and bg colors.
   * This new win will be the current default. */
  qsWin_create(); // another window is the new default win.
